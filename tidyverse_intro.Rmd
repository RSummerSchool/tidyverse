...
* `f(x, y = nrow(x), z = ncol(x))` is equivalent to 
`x %>% f(y = nrow(.), z = ncol(.))`
...
* Select all variables that `ends_with` "n"
...
* Filter for Pokemon that do not have a gender (`male` = "")
* How many Pokemon do only have one type? (`type_2` is NA)
* Filter for Pokemon with the same name in English and German
  * from generation 1
* Pokemon that have a size between 0.5 and 1
