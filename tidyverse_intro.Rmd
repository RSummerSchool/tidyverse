...
This operator take the left hand side and _plugs_ it into the right hand side as first argument.

* `f(x)` equivalent to `x %>% f()`
* `f(x, y)` is equivalent to `x %>% f(y)`
* `h(g(f(x)))` is equivalent to `x %>% f %>% g %>% h`

This operator make code much more readable as we will see throuout this presentation.
...
If your data is saved in database [`dbplyr`](https://github.com/tidyverse/dbplyr) is the tidyverse database backend (e.g. for MySQL/MariaDB, Postgresql and SQLite).
...
> Important: To make your research reproduciable and be sure the next time you import the file it 
has the same format: specify the col_types explicitly as follows
...
The Visualise part will be done in the other session about `ggplot2`
...
* Select only the names (german and english)
* Select all variables that start_with `e`
* Select the names, attack, speed and size
...
You can combile filters with AND `&` and OR `|`. Filter expects a logical vector of the same length as the data:
...
* Filter for Pokemon that do not have a gender (`male` is NA)
* How many Pokemon do only have one type?
* Filter for Pokemon with the same name in english and german from generation 1 and type_1 == Gift
...
* Compute the difference between attack and defense and name it `diff_att_def`
...
Group by also works with `mutate`. Lets say we want to know how much the maximum hp is bigger than the mean hp:
...
* Which Pokemon has the highest `speed` per `type_1`?
* How much more/less `attack` do Pokemon with a dual_type have?
* Calculate the mean for all numeric variables per generation. Carefully read the documentation of `summarise_if`
...
gen <- pokemon2$generation %>% unique
for (i in seq_along(gen)) {
  pokemon %>% 
    filter(generation == gen[i]) %>% 
    write_csv(path = paste0("dat/multi-file/pokemon_raw_gen_", i, ".csv"))
}
```
...
And now we want to fit the same model per generation. Do do this we nest the data as follows:
```{r}
